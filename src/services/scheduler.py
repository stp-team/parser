...
from src.services.parser_events import run_tracked_task
...
class Scheduler:
    def __init__(
        ...
        Args:
            ...
            db_url: URL на БД для сохранения задач
            max_workers: Максимальное количество задач
        """
    ...
    async def stop_scheduling(
        ...
        try:
            ...
            if "default" in self.scheduler._jobstores:
                jobstore = self.scheduler._jobstores[
                    "default"
                ]
    ...
    async def _setup_employees(
        ...
        dossier_hours_expr = ",".join(
            str(hour)
            for hour in dossier_hours
        )
        ...
        self.scheduler.add_job(
            self._safe_job_wrapper(
                fill_employee_ids,
                "employees_ids",
                "Заполнение идентификаторов OKC",
            ),
            trigger=CronTrigger(
                hour=dossier_hours_expr,
                minute=0,
            ),
            args=[
                self.okc_client.api.dossier,
            ],
            id="employees_ids",
        ...
        self.scheduler.add_job(
            self._safe_job_wrapper(
                fill_employment_dates,
                "employees_employment_dates",
                "Заполнение дат трудоустройства",
            ),
            trigger=CronTrigger(
                hour=dossier_hours_expr,
                minute=20,
        ...
        self.logger.info(
            "[Планировщик] Задачи сотрудников "
            "настроены на часы: %s",
            dossier_hours,
        )

    async def _setup_kpi(
        self,
    ) -> None:
        self.scheduler.add_job(
            self._safe_job_wrapper(
        ...
        self.logger.info(
            "[Планировщик] Задачи KPI настроены"
        )

    async def _setup_premium(
        self,
    ) -> None:
        self.scheduler.add_job(
            self._safe_job_wrapper(
        ...
        self.logger.info(
            "[Планировщик] Задачи премиума настроены"
        )

    async def _setup_tutors(
        self,
    ) -> None:
        async def update_tutors(api):
            return await fill_tutor_schedule(
                api,
                full_update=False,
            )
        ...
        self.logger.info(
            "[Планировщик] Задачи наставников настроены"
        )

    async def _setup_tests(
        self,
    ) -> None:
        self.scheduler.add_job(
            self._safe_job_wrapper(
        ...
        self.logger.info(
            "[Планировщик] Задачи тестов настроены"
        )

    def _safe_job_wrapper(
        self,
        job_func,
        job_name: str,
        job_title: str,
